    ...
    private final Runnable discoverRunnable = new Runnable()
    {
        ...
        public void run()
        {
            ...
            try
            {
                ...
                byte[] helloPacket = TPLDatagrammService.encryptMessage(mess);
            ...
            if (socket != null)
            {
                ...
                while ((discoverThread != null) && (System.currentTimeMillis() < exittime))
                {
                    try
                    {
                        ...
                        String messstr = TPLDatagrammService.decryptMessage(packet.getData(), 0, packet.getLength());
                        ...
                        TPLHandlerSysInfo.buildDeviceDescription(ipaddr, ipport, message);
                    }
                    catch (Exception ignore)
                    {
                    }
